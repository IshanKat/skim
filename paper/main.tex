...
once per question. The policy runs on pre-cached embeddings (one VLM call per episode,
so PPO trains on a single $8$\,GB GPU), is \emph{causal} (its state never depends on
future frames, making it streaming-compatible), and uses a learned \textsc{stop} with a
cost-aware reward to set a per-question \emph{variable} budget---a combination that
concurrent RL frame selectors, which are offline and fixed-budget, do not share. On
NExT-QA, SKIM matches Uniform-16 accuracy ($76.0\%$) using fewer frames ($14.0$ vs.\
$15.7$); a single coefficient $\lambda$ trades accuracy for frames; and the policy
transfers zero-shot to IntentQA. We also analyze how it fails: under a strong frame
penalty it reward-hacks, stopping with near-zero frames and answering from the VLM's
language prior.
\end{abstract}
...
frames (Uniform-8/16/32) to the same frozen VLM. We report accuracy, the average
number of frames passed to the VLM, and per-question-type accuracy on $n=200$
validation subsets. All runs use a single RTX 2070 (8\,GB).

\begin{table}[t]
  \caption{Results on NExT-QA (val, $n=200$). SKIM ($\lambda{=}0.05$) matches
    Uniform-16 accuracy using fewer frames; raising $\lambda$ trades accuracy
    for frames, and at $\lambda{=}0.2$ the policy collapses to near-zero frames
    (Section~\ref{sec:analysis}). Best SKIM accuracy in \textbf{bold}.}
  \label{tab:nextqa}
  \centering
  \begin{tabular}{lccccc}
    ...
    \midrule
    SKIM ($\lambda{=}0.05$) & \textbf{76.0} & 14.0 & 75.0 & 73.6 & 85.7 \\
    SKIM ($\lambda{=}0.1$)  & 74.5 & 8.6  & 76.0 & 68.1 & 85.7 \\
    SKIM ($\lambda{=}0.2$)  & 58.5 & 1.5  & 57.0 & 54.2 & 75.0 \\
    \bottomrule
  \end{tabular}
\end{table}

\begin{table}[t]
  \caption{Zero-shot transfer to IntentQA (val, $n=200$); the selector is trained
    only on NExT-QA. SKIM ($\lambda{=}0.05$) matches Uniform-32 accuracy using
    far fewer frames.}
  \label{tab:intentqa}
  \centering
  \begin{tabular}{lcccc}
...
causal/temporal/descriptive. The distribution is bimodal, with one mode at
near-immediate stopping and another at running to the forced stop, but the balance
shifts with the question. Descriptive
...
telling. When it does, SKIM answers correctly $93.3\%$ of the time using only $7.1$
frames; when it never fires and the episode runs to the forced stop, accuracy falls to
$46.2\%$ at $13.0$ frames. Early stopping is thus not the cause of descriptive
errors---if anything, the cases where the policy \emph{cannot} commit to a stop are the
genuinely hard ones it would miss regardless. The aggregate descriptive number in
Table~\ref{tab:nextqa} ($85.7\%$) mixes these two populations.
...
Our evaluation uses $n{=}200$ validation subsets and a single training run per
...
should be read as indicative rather than significant. We use a single frozen backbone
(Qwen2.5-VL-3B in 4-bit) on one consumer GPU and have not tested whether the learned
policy transfers across VLM scales or families. Most importantly, although SKIM's
causal design makes it \emph{compatible} with streaming, all of our experiments are
offline---finite clips capped at $32$ frames with a forced stop at the end---so we do
not evaluate a true streaming deployment, and we leave EgoSchema and
full-validation-set runs to future work.
